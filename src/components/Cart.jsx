  ...
  return (
    ...
    >
      <div
        ...
        className=" relative z-20 bg-slate-300 text-slate-900 min-w-[400px] md:min-w-[700px] px-10 pt-10 pb-6 rounded border-slate-600 mb-[10vh]] "
      >
        <button
          onClick={onClose}
            ...
            cart.cartItems.map((el) => (
              <li key={el.id} className="flex items-center mb-4">
                <img
                  ...
                  alt={el.title}
                />
                <p className="mr-auto ml-2 text-lg text-semibold">{el.title}</p>
                <select
                  name="quantity"
                  onChange={(e) =>
                    dispatch(
